...
func (c *Client) SaveStateData(ctx trace.SpanContext, store string, data *StateData) error {
	list := []*StateData{data}
...
func (c *Client) SaveState(ctx trace.SpanContext, store, key string, in interface{}) error {
	state := &StateData{
		Key:   key,
		Value: in,
